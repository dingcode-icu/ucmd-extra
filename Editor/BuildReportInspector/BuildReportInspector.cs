...
namespace Unity.BuildReportInspector
{
    ...
    public class BuildReportInspector : Editor {
         ...
         * 打印不满足大小的 texture
         */
        public static string getTextureForMax()
        {
            ...
            var assetsTemp = new List<AssetEntry>();

            try
            {
                BuildReport report = AssetDatabase.LoadAssetAtPath<BuildReport>(assetPath) as BuildReport;
                ...
                assetsTemp = assetsTemp.OrderBy(p => -p.size).ToList();


                string strPostDesc = "";
                for (int i = 0; i < assetsTemp.Count; i++)
                {
                    if (assetsTemp[i].size > 4*1024*1024 + 200)
                    {
                        if (nameof(UnityEngine.Video.VideoClip).Equals(assetsTemp[i].type) 
                            || nameof(UnityEngine.Font).Equals(assetsTemp[i].type))
                        {
                            continue;
                        }
                        Debug.LogError("" + Convert.ToInt32(assetsTemp[i].size / 1024 / 1024) + "M " + assetsTemp[i].path);
                        strPostDesc += "" + Convert.ToInt32(assetsTemp[i].size / 1024 / 1024) + "M " +
                                       assetsTemp[i].path + "\n";
                    }
                }
